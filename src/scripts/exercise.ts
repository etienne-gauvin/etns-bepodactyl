...
export default class Exercise {
  ...
  public constructor(id: number, data: IExerciseData, session: Session) {
    ...
    data.sentences.forEach((text, id) =>
      this.sentences.push(new Sentence(id, text, this))
    );
  }

  public get correct(): boolean | null {
    const notCorrectSentence = this.sentences.find((sentence) =>
      !sentence.correct
    );
  ...
  public get previous(): Exercise {
    return getPrevious(this.session.exercises, this) ||
